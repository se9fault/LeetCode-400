...
public:
    int myAtoi(string str) {
        int result = 0;
        bool signProcessed = false; // sign processed or not
        bool sign = false;          // positive: 0, negative: 1
        ...
        for (; i < str.length(); ++i) {
            ...
            if (isdigit(str[i])) {
                if (!sign) {
                    if (result > INT_MAX/10 || (result == INT_MAX/10 && int(str[i]-'0') >= 7)) {
                        return INT_MAX;
                    }
                } else {
                    if (result > INT_MAX/10 || (result == INT_MAX/10 && int(str[i]-'0') >= 8)) {
                        return INT_MIN;
                    }
                }
